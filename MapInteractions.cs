    ...
    public partial class Main
    {
            ...
            try
            {
                ...
                if (speed != Enums.Speed.Slip)
                {
                    ...
                    if (stepCounter % 5 == 0)
                    {
                        ...
                        if (character.HasActiveItem(298) && (character.CharacterType == Enums.CharacterType.MapNpc || ((Recruit)character).Belly > 0))
                        {//heal ribbon
                            h += 150;
                        }
                        if (HasActiveBagItem(character, 12, 0, 0) && (character.CharacterType == Enums.CharacterType.MapNpc || ((Recruit)character).Belly > 0))
                        {//heal ribbon
                            h += 150;
                        }
            ...
            else
            {
                ...
                switch (script)
                {
                    ...
                    case 28:
                        ...
                        break;
                    case 34:
                        {//secret (Stairs, etc), not used here
                        ...
                        {//pitfall
                            ...
                            for (int i = 0; i < targets.Count; i++)
                            {
                                if (targets[i].CharacterType == Enums.CharacterType.Recruit)
                                {
                                    ...
                                    try
                                    {
                                        if (map.Tile[x, y].String3 == "" && map.MapType == Enums.MapType.RDungeonMap)
                                        {//fall down a level in a random dungeon
                                            if (RDungeonManager.RDungeons[((RDungeonMap)map).RDungeonIndex].Direction == Enums.Direction.Up)
                                            {//fall down a level
            ...
            try
            {
                switch (script)
                {
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        {//chamber tile; doesn't do anything here
                    ...
                    case 22:
                        {
                            ...
                            if (zoneID <= 0)
                            {
                                Story story = new Story();
                                StoryBuilderSegment segment = StoryBuilder.BuildStory();
                                StoryBuilder.AppendSaySegment(segment, "The sign is empty.", -1, 0, 0);
                                segment.AppendToStory(story);
                                StoryManager.PlayStory(client, story);
                            }
                            else
        ...
        public static RDungeonChamberReq GetChamberReq(int chamberNum, string string1, string string2, string string3)
        {
            ...
            try
            {
                ...
                {
                    case 1:
                        {//pre-mapped, does not allow start or end
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        {//chamber that accepts start and end
